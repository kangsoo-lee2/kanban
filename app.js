...
async function loadCards() {
  ...
  if (error) { console.error('카드 로드 실패:', error); return; }
...
async function addCard(col) {
  ...
  if (error) { console.error('카드 추가 실패:', error); return; }
  input.value = '';
  await loadCards();
}
...
async function deleteCard(id) {
  ...
  if (error) { console.error('카드 삭제 실패:', error); return; }
  await loadCards();
}
...
async function moveCard(id, destCol) {
  ...
  if (error) { console.error('카드 이동 실패:', error); return; }
  await loadCards();
}
